<template>
	<div>
		<div v-if="isLogin" class="class5">
			<el-alert title="账号未登录" type="warning" description="请前往主页面登录" show-icon>
			</el-alert>
		</div>
		<div>
			<el-descriptions direction="vertical" :column="6" border>
				...
				<el-descriptions-item label="最大容量/人" :span="1.5">{{sceniccapacity}}</el-descriptions-item>
				<el-descriptions-item label="当前人数/人" :span="1.5">{{sceniccurrnum}}</el-descriptions-item>
		...
		<div class="class1">
			...
			<div class="comforstyle">
				<el-progress :text-inside="true" :stroke-width="22" :percentage="sceniccomfor"
					:status="sceniccomforstatue"></el-progress>
			</div>
		</div>
		<div class="class2">
			<el-row>
		...
		<div>
			...
			<div>
				<div v-if="typyid == 1">
					<el-table :data="moneyList.slice((currentPage - 1) * pagesize, currentPage * pagesize)" stripe
						style="width: 100%" ref="recordTable" :height="tableHeight" id="table">
						<el-table-column width="100px;" align="center" prop="moneyId" label="序号">
						</el-table-column>
						<el-table-column align="center" prop="ticketUsetimeYear" label="年份">
						</el-table-column>
						<el-table-column align="center" prop="ticketUsetimeMonth" label="月份">
						</el-table-column>
						<el-table-column align="center" prop="ticketUsetimeDay" label="日期">
						</el-table-column>
						<el-table-column align="center" prop="money" label="销售额">
						</el-table-column>
					</el-table>
				</div>
			...
			<template>
				<div class="block">
					<el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
						...
						layout="total, sizes, prev, pager, next, jumper" :total="moneyList.length">
					</el-pagination>
				</div>
			</template>
		</div>
...
<script>
	...
	import {
		getTicketMoneyByMonth
	} from '@/api/index.js';
	import {
		getLatestFiveYearTicketMoney
	} from '@/api/index.js';
	import {
		staffCountScenicIn
	} from '@/api/index.js';
	...
	export default {
		...
		data() {
			return {
				...
				isLogin: false,
				//--------------------检测是否登录-------------------------
				belongScenicName: window.localStorage.getItem.scenicName, //接收页面跳转的该值班人员对应的景区名称
				belongscenicinfo: [],
				//----------------------------接收传参-------------------------
				scenicname: '',
				sceniclocation: '',
		...
		methods: {
			...
			CountScenicIn() {
				...
				sGetScenicInfo(data).then(res => {
					if (res != -1) {
						console.log("景区id");
						console.log(res.data.datas.scenicId);
				...
				if (!this.belongScenicName || this.belongScenicName == '没有') {
					...
					sGetScenicInfo(data).then(res => {
						if (res != -1) {
							...
							this.sceniccomfor = parseInt(res.data.datas.scenicCurrentNumber) / parseInt(res.data
								.datas.scenicCapacity) * 100;
