...
<script>
	import { searchForAllVisitors, searchByName, searchByPhone, searchByScenic, searchByUseTime} from '@/api/index.js';
	export default {
		name: 'cVisiters',
		data(){
			return{
		...
		methods:{
			...
			search(){
			  ...
			  switch (this.stringType){
			  	case "游客姓名":
				{
					searchByName({userName: this.string}).then((res) => {
						if(res != -1){
							this.visiterList = res.data.datas;
							for(let i = 0; i < this.visiterList.length; i++)
							{
				...
				case "手机号":
				{
					searchByPhone({userPhone: this.string}).then((res) => {
						if(res != -1){
							this.visiterList = res.data.datas;
							for(let i = 0; i < this.visiterList.length; i++)
							{
				...
				case "景区名称":
				{
					searchByScenic({scenicName: this.string}).then((res) => {
						if(res != -1){
							this.visiterList = res.data.datas;
							for(let i = 0; i < this.visiterList.length; i++)
							{
