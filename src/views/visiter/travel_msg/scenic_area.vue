<template>
	<div>
		...
		<el-drawer
		  ...
		  <div>
			<div style="margin-top: 8px;margin-bottom: 18px;margin-left: 20px;">
				当前所选景区
				
				<span style="margin-left: 20px;">{{draw.name}}</span>
				
			</div>
			<!--买票时间-->
		     <div class="block" style="margin-top: 8px;margin-bottom: 18px;margin-left: 20px;">
			...
			<div style="margin-top: 28px">
				<span class="demonstration" style="margin-top: 8px;margin-bottom: 18px;margin-left: 20px;">购票张数</span>
				<i class="el-icon-remove-outline" @click="declineCount" style="margin-left: 50px;margin-right: 10px; margin-top: 2px;
				 font-size: 20px;"></i>
				{{draw.ticketCount}}
				<i class="el-icon-circle-plus-outline" @click="addCount" style="margin-left: 10px; margin-top: 2px; font-size: 20px;"></i>
			</div>
			<!--总价-->
			<div style="margin-top: 28px;margin-bottom: 18px;margin-left: 20px;">
				总价
			...
			<!--确认购买-->
			<el-button type="danger" style="margin-top: 18px;margin-bottom: 18px;margin-left: 20px;" @click="buyTicket">确认购买</el-button>
		  </div>
		</el-drawer>
	</div>
</template>
	  ...
	  methods:{
		  ...
		  buyTicket(){
			 ...
			 for (let i = 0; i < this.draw.ticketCount; i++) {
				this.tempdata= {
					   user_id: "钟离的狗",
					   scenic_name: this.draw.name,
					   ticket_buy_time: year +
										"-" + 
										month +
										"-" +
										strDate,
					   ticket_use_time: this.draw.buyDate,
								
						ticketPrice: this.draw.ticketPrice

				   };
			       this.buyTicketInfo.push(this.tempdata);
			}
			console.log(this.buyTicketInfo);
			this.buyTicketInfo = [];
			this.tempdata = "";
		  },
		  initSceni(){
			  ...
			  this.indexdlt = this.scenies.findIndex(item => item.scenicname=='大礼堂');
			  this.indexdzs = this.scenies.findIndex(item => item.scenicname=='弹子石');
			  ...
			  this.indexzms = this.scenies.findIndex(item => item.scenicname=='照母山');
			  this.indexjfb = this.scenies.findIndex(item => item.scenicname=='解放碑');
			  this.indexlsm = this.scenies.findIndex(item => item.scenicname=='烈士墓');
